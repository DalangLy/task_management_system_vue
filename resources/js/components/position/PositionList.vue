<template>
    <div>
        ...
        <div class="d-flex flex-row justify-content-between align-items-center my-3">
            <button @click.prevent="$router.push({name: 'positions.create'})" class="btn btn-primary"><i class="fas fa-plus"></i></button>
        ...
        <table class="table table-bordered">
            ...
            <tbody>
            <tr v-for="(position, index) in allData">
                <th class="align-middle" scope="row">{{(index+1)}}</th>
                <td class="align-middle">{{position.position}}</td>
    ...
    export default {
        ...
        methods:{
            ...
            filterSearch(e){
                ...
                let found = this.positions.filter(function(element) {
                    return (element.position+'').toString().toLowerCase().includes(keyword.toLowerCase());
                });
                if(keyword.length <= 0){
                    this.searchResult = [];
                }
